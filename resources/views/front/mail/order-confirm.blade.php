...
@php
    $orderDetails = \App\Models\Order::where('order_no', $orderNo)->first();
    $totalAmount = $orderDetails->amount;
    $finalAmount = $orderDetails->final_amount;

    if($orderDetails->coupon_code_id == 0) {
        $couponUsage = 0;
        $couponDiscountType = '';
        ...
        $couponUsage = 1;
        $couponDiscountType = $orderDetails->coupon_code_type;
        $couponDiscountAmount = $orderDetails->discount_amount;
        $couponDiscountDesc = ($orderDetails->coupon_code_type == 'voucher') ? $orderDetails->discount_amount.'% OFF' : '&#8377;'.$orderDetails->discount_amount.' OFF';
    }
@endphp
													...
													<table width="100%" border="0" cellspacing="0" cellpadding="0">
														...
														<tr>
															<td class="text pb15" style="color:#2e364b; font-family:Arial, sans-serif; font-size:14px; line-height:24px; text-align:left; padding-bottom:8px;">
                                                                ...
                                                                @if($couponUsage == 1)
                                                                <li style="font-size: 14px; font-family: Arial, sans-serif; line-height: 20px; color: #2e364b; margin-bottom: 5px;">
																	Coupon discount : <b>{!! $couponDiscountDesc !!}</b>
																</li>
                                                                @endif
																@if($orderDetails->address_type=='ho')
																<li style="font-size: 14px; font-family: Arial, sans-serif; line-height: 20px; color: #2e364b; margin-bottom: 5px;">
																	Value of Order : <b> &#8377; {{($orderDetails->final_amount)-($orderDetails->shipping_charges)}}</b>
																</li>
																@elseif($orderDetails->address_type=='dankuni')
																	<li style="font-size: 14px; font-family: Arial, sans-serif; line-height: 20px; color: #2e364b; margin-bottom: 5px;">
																	Value of Order : <b> &#8377; {{($orderDetails->final_amount)-($orderDetails->shipping_charges)}}</b>
																</li>
																	@else
																	<li style="font-size: 14px; font-family: Arial, sans-serif; line-height: 20px; color: #2e364b; margin-bottom: 5px;">
																	Value of Order : <b> &#8377; {{$finalAmount}}</b>
																</li>
																	@endif
																</ul>
															</td>
															...
															<table width="550" border="0" cellspacing="0" cellpadding="0" class="amttable">
																...
																<tbody>
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <td style="color:#000; font-family:Arial, sans-serif; font-size:14px; line-height:20px; text-align:center; padding:4px; font-weight:400">&#8377; {{$bookingProductVal->qty * $bookingProductVal->offer_price}}</td>
                                                                    </tr>
                                                                    @endforeach
																</tbody>
															</table>
