        ...
        <div class="row">
            <div class="col-md-7">
                <div class="store_image">
					...
					@else
                    <img src="{{ asset('img/store_placeholder.png') }}" alt="" class="w-100">
					@endif
                </div>
            </div>
            <div class="col-md-5">
                <div class="store_info">
                    ...
                    <div class="d-flex">
                        <a href="#noOrderModal" data-toggle="modal" class="btn darkBlueBtn mr-3">No order</a>
                        <a href="{{ route('front.user.order', ['store' => $data->id]) }}" class="btn sweetRedBtn">Order History</a>
                        {{-- <a href="{{route('front.store.order.history', $data->id)}}" class="btn sweetRedBtn">Order History</a> --}}
                    </div>
                </div>
            </div>
        </div>
...
<section class="storeProducts py-3">
                            ...
                            <div class="col-sm-6 col-md-6 col-lg-4 storeProduct_col product_{{ $categoryProductValue->category->slug }}_{{$item->slug}}">
                                <div class="storeProduct_card">
                                    <div class="storeProduct_card_body">
                                        ...
                                        <a href="javascript: void(0)" data-toggle="modal" class="product__single" data-events data-cat="tshirt" onclick="proDetail('{{$categoryProductValue->slug}}', {{$data->id}}, '{{$type}}')">
...
{{-- no order modal --}}
<div id="noOrderModal" class="modal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">No Order Reason</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                @php
                    $noOrderData = DB::select('select * from no_order_reasons');
                @endphp

                <form class="modalFormField" action="{{route('front.store.noorder')}}" method="POST">@csrf
                    <div class="row">
                        <div class="col">
                            <select name="comment" id="" class="form-control" onchange="noOrderReasonOptions($(this).val())">
                                @foreach ($noOrderData as $reasonKey => $reasonValue)
                                    <option value="{{$reasonValue->noorderreason}}">{{$reasonValue->noorderreason}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-auto">
                            ...
                            <button type="submit" class="btn sweetRedBtn sweetRedBtnsm">Save changes</button>
                        </div>
                    </div>
                </form>
            </div>
...
@section('script')
    <script>
        // no order reason
        function noOrderReasonOptions(val) {
            // alert(val);
            if(val == "More Factor") {
                $('.reasonDiv').show();
                    ...
                    success: function(result) {
                        ...
                        if (result.error === false) {
                            ...
                            $.each(result.data, (key, value) => {
                                ...
                                content += `<a class="dropdown-item" href="javascript: void(0)" onclick="proDetail('${value.slug}', ${value.id}, '${type}')">
                                    <div class="d-flex">
                        ...
                        content += `</ul>
                            ...
                            $.each(result.response.primaryColorSizes[0].primarySizes, function(sizesKey, sizesValue) {
                                ...
                                <div class="col-xl-6 col-lg-8pu col-6">
                                    <div class="sizeBox single_size_box">
                        ...
                        content += `</div>
                            <div class="row">
                                <div class="col-xl-4 col-lg-6 col-md-6 col-6">
                                    ...
                                    <button type="submit" class="btn btn-block addtocartBtn" disabled>
        ...
        /* $(document).on('keyup', '.qqttyy', () => {
            $('.qqttyy').each(function() {
                var minVal = $('input[name="master_pack_count"]').val();
                var val = $(this).val();
