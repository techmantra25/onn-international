...
<div class="col-sm-12">
    <div class="profile-card">
        <h3 class="mb-0">Product wise Report</h3>
                                ...
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        ...
                                        if ( request()->input('from') || request()->input('to') ) {
                                            ...
                                            $report = \DB::select("SELECT op.product_name, op.product_id, p.style_no, SUM(op.qty) AS product_count FROM `order_products` op
                                            INNER JOIN products p ON p.id = op.product_id
                                            INNER JOIN orders o ON o.id = op.order_id
                                            INNER JOIN stores s ON s.id = o.store_id
                                            WHERE s.user_id = ".auth()->guard('web')->user()->id." 
                                            ...
                                            /* $report = DB::select("SELECT op.product_name, op.product_id, p.style_no, SUM(op.qty) AS product_count FROM `orders` o
                                        ...
                                        @endphp
                                          @foreach($report as $reports)
                                            <tr>
                                                <td>
                                                    <p class="mb-0">Style no #{{$reports->style_no}}</p>
                                                </td>
                                                <td>
                                                    <p class="small mb-0 text-muted">{{$reports->product_name}}</p>
                                                </td>
                                                <td>
                                                    <p class="mb-0">
                                                        {{ ($reports->product_count == null) ? 0 : $reports->product_count }}</p>
                                                </td>
                                            </tr>
                                            @endforeach
