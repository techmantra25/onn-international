                                ...
                                <tbody>
                                    @foreach ($data as $item)
                                        <tr>
                                            <td>
                                                @if (request()->input('rsm'))
                                                    <a href="{{ route('front.sales.report.detail', ['asm' => ($item->name == null) ? 'NA' : $item->name, 'state' => request()->input('state')]) }}">{{ ($item->name == null) ? 'NA' : $item->name }}</a>
                                                @elseif (request()->input('asm'))
                                                    <a href="{{ route('front.sales.report.detail', ['ase' => ($item->name == null) ? 'NA' : $item->name, 'state' => request()->input('state')]) }}">{{ ($item->name == null) ? 'NA' : $item->name }}</a>
                                                @elseif (request()->input('ase'))
                                                    <a href="{{ route('front.sales.report.detail', ['distributor' => ($item->name == null) ? 'NA' : $item->name, 'state' => request()->input('state')]) }}">{{ ($item->name == null) ? 'NA' : $item->name }}</a>
                                                @elseif (request()->input('distributor'))
                                                    <a href="{{ route('front.user.order', ['store' => $item->store_id, 'store_name' => ($item->name == null) ? 'NA' : $item->name]) }}">{{ ($item->name == null) ? 'NA' : $item->name }}</a>
                                                    {{-- {{ ($item->name == null) ? 'NA' : $item->name }} --}}
                                                @endif
                                            </td>
                                            @if (auth()->guard('web')->user()->user_type == 4)
                                                @if (request()->input('ase'))
                                                    <td>{{$item->qty}} {{ ($item->qty == 1) ? 'pc' : 'pcs' }}</td>
                                                @endif
