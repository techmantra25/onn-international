        ...
        <div class="product-details__content__holder">
            ...
            <div class="product__pricing">
                ...
                @if (count($data->colorSize) > 0)
                    ...
                    @endphp
                   MRP &#8377; <span class="price_val">{{$displayPrice}}</span>
                @else
                   MRP &#8377;  <span class="price_val">{{$data->offer_price}}</span>
                @endif

                    <p class="tax-class">Inclusive of all taxes</p>
                </h3>
            </div>
			...
			<div class="specification_block">
				<h6 class="spec_head">Details & Specifications</h6>
				...
				<h6 class="spec_head">More Information</h6>
						<div class="specification">
				...
				{{--<h6 class="spec_head">Care</h6>
				<div class="specification">
					<p>Mild Wash</p>
					<p>Warm Iron On Reverse</p>
					<p>Wash similar colour together only</p>
					{!! substr($data->desc, 0, 100) !!}
					
				</div>--}}
				<h6 class="spec_head">Customer Care</h6>
				<div class="specification">
					<div class="mb-4">Customer care details :</div>
					<p class="mb-2">E-Mail : support@onninternational.com</p>
					<p class="mb-2">Address : 39  KALI KRISHNA TAGORE STREET, KOLKATA - 700007</p>
					<p class="mb-2">SMS  or Call to 9123954481</p>
					{{--<h6><a href="javascript: void(0)" data-bs-target="#productDescModal" data-bs-toggle="modal">Read more</a></h6>--}}
				</div>
			</div>
        </div>
...
<div class="modal fade" id="sizeChartModal">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h6>Size Chart For {{$data->name}}</h6>
                <button type="button" class="btn btn-close" data-bs-dismiss="modal" aria-label="Close">&times;</button>
            </div>
            <div class="modal-body">
                <figure>
...
<script>
	...
	$('#addToCartForm').on('submit', function(e) {
        ...
        if ($('input[name="user_id"]').val() == 0) {
            ...
            const tokenExists = getCookie('cartToken');

            if (!tokenExists) {
                token = '{{generateUniqueAlphaNumeric(10)}}';
                setCookie('cartToken', token, 30);
			...
			success: function(result) {
				...
				if (result.status == 200) {
					...
					if (result.message == "wishlisted") {
                        toastFire('success', 'Product addded to Wishlist');
						// Toast.fire({
						//   icon: 'success',
                        ...
                        toastFire('warning', 'Product removed from Wishlist');
						// Toast.fire({
						//   icon: 'success',
