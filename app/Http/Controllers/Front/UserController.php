...
class UserController extends Controller
{
    ...
    public function register1_create(Request $request)
    {
        $request->validate([
        ...
        try {
            ...
            $user->password = Hash::make($randomPassword); // SAVE HASHED PASSWORD
            $user->save();
            ...
            // Move any guest cart to this user (optional)
            if (!empty($_COOKIE['cartToken'])) {
                Cart::where('guest_token', $_COOKIE['cartToken'])
            ...
            return redirect()->intended('home')
                ->with('success', 'Account created & logged in successfully');
    
        } catch (\Throwable $e) {
            DB::rollback();
            dd($e->getMessage());
            return redirect()->back()->withInput()
                ->with('failure', 'Something went wrong: ' . $e->getMessage());
        }
    }
